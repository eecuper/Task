        ...
        launchMobileChat: function(){
            ...
            kfuinCache.get(nameAccount, function(kfuin){
                ...
                var dm = domain.domain,
                    crmSchema = 'mqqwpa://im/chat?chat_type=crm&uin=' + kfuin + '&version=1&src_type=web&web_src=http://' + dm,
                    schema = 'mqqwpa://im/chat?chat_type=wpa&uin=' + kfuin + '&version=1&src_type=web&web_src=http://' + dm,
                    start = +new Date(),
                ...
                if(browser.isQQ){
                    // QQ version 4.5 & 4.6 use schema
                    // otherwise use CRM schema
                    // no fallback because it's in QQ already
                    // fallback would mislead page
                    launch(/(?:4\.5|4\.6)/.test(browser.QQVersion) ? schema : crmSchema);
                    return;
                }

                // when running in standard browser
                // no QQ info known
                // try CRM schema first
                // and fallback to schema when failed
